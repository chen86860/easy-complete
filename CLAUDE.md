...
**Config + pinning.** All spec-source settings live in `specs.config.json` at the repo root — `repo`, `tag`, and `exclude` — so they are edited and reviewed without touching the script. The `tag` is pinned to a specific release (currently `spec-build-number-0.1.0`), **not `latest`**, so the bundle changes only when the config changes, never silently. To adopt a newer fork build: bump `tag` in `specs.config.json`, re-run the sync, and commit the regenerated `bundle/specs` alongside it (one reviewable change). Env overrides still win: `BUNDLED_SPECS_TAG=<tag|latest>`, `BUNDLED_SPECS_EXCLUDE=<csv>`, `BUNDLED_SPECS_RELEASE_ZIP=<full-url>`, or `BUNDLED_SPECS_RELEASE_ZIP=""` to fall back to the legacy per-file CDN sync (`https://specs.q.us-east-1.amazonaws.com/`, frozen 2025-05-05).
